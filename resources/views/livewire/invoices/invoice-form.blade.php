...
new class extends Component {
    ...
    // Items Repeater
    public array $items = []; // Array of ['product_id', 'quantity', 'unit_price', 'subtotal']

    public function rules(): array
    {
        return [
            'client_id' => ['required', 'exists:clients,id'],
            'invoice_number' => ['required', 'string', 'max:255', Rule::unique('invoices')->ignore($this->invoice?->id)],
        ...
        <div class="bg-white p-6 rounded-lg shadow space-y-4">
            <div class="flex justify-between items-center border-b pb-2">
                ...
                <button type="button" wire:click="addItem"
                    class="px-3 py-1 bg-green-600 text-white text-sm rounded hover:bg-green-700">Add Item</button>
            </div>
                ...
                @foreach($items as $index => $item)
                    <div class="grid grid-cols-12 gap-4 items-end border-b pb-4 last:border-0 last:pb-0">
                        <div class="col-span-5">
                            ...
                            @error("items.{$index}.product_id") <span class="text-red-500 text-xs">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="col-span-3">
                            ...
                            <input wire:model.live="items.{{ $index }}.unit_price" type="number" step="0.01" min="0"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            @error("items.{$index}.unit_price") <span class="text-red-500 text-xs">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="col-span-1">
                            <button type="button" wire:click="removeItem({{ $index }})"
                                class="mb-1 text-red-600 hover:text-red-900 p-2">
